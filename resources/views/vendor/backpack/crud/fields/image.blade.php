  <div class="form-group col-md-12 image" data-preview="#{{ $field['name'] }}" data-aspectRatio="{{ isset($field['aspect_ratio']) ? $field['aspect_ratio'] : 0 }}" data-crop="{{ isset($field['crop']) ? $field['crop'] : false }}" @include('crud::inc.field_wrapper_attributes')>
    ...
    <div class="row">
        <div class="col-sm-6" style="margin-bottom: 20px;">
            <?php $field_image_url_only_for_view = (old($field['name']) ? old($field['name']) : (isset($field['value']) ? $field['value'] : (isset($field['default']) ? $field['default'] : '') )) ?>
            <img id="mainImage" src="{{ url( (isset($field['prefix']) ? $field['prefix'] : '') . $field_image_url_only_for_view) }}" style="max-width: 150px; max-height: 150px; @if (empty($field_image_url_only_for_view)) display:none; @endif ">
        </div>
                        ...
                        if (/^image\/\w+$/.test(file.type)) {
                            ...
                            fileReader.onload = function () {
                                ...
                                if(crop){
                                    ...
                                    $mainImage.attr('src',this.result).css('display', 'block');
